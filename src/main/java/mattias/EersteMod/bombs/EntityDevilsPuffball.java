...
public class EntityDevilsPuffball extends ThrowableEntity implements IRendersAsItem {

	public EntityDevilsPuffball(EntityType<? extends ThrowableEntity> entity, World worldIn) {
		super(entity, worldIn);
	}

	public EntityDevilsPuffball(World worldIn, LivingEntity shooter) {
		super(RegistryHandler.DEVILS_PUFFBALL_ENTITY.get(), shooter, worldIn);
			...
			for (LivingEntity target : entities) {
				...
				if (distanceSq < 16.0D) { // within 4 blocks
					...
					if (duration > 20) { // must last at least 1 sec
						target.addPotionEffect(new EffectInstance(Effects.POISON, duration, 0, false, true));
					}
				}
			}
		}
	...
	public ItemStack getItem() {
		return new ItemStack(RegistryHandler.DEVILS_PUFFBALL.get());
	}
